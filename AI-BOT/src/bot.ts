...
class DiscordBot {
  ...
  constructor(private configService: ConfigService = new ConfigService()) {
    this.TOKEN = configService.discord.token;
    this.HISTORY_MODE = configService.bot.historyMode;
    this.MAX_MESSAGE_LENGTH = configService.bot.maxMessageLength;
    this.MESSAGE_CONTENT_ALLOWED = configService.bot.messageContentAllowed;
    this.TRIGGER_KEYWORDS = configService.bot.triggerKeywords;

    if (!this.TOKEN) {
      throw new Error("Discord token must be provided in config.json");
    }
